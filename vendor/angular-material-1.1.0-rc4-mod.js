...
/**
  * Баги Angular Material:
  * 1) При вводе пробела модель (md-search-text - self.searchText) не меняется, т.к. по умолчанию работает trim
  *  https://github.com/angular/material/issues/4492
  *  https://github.com/angular/material/issues/7140
  *  Патч: в шаблоне function getInputElement () класса MdAutocomplete добавить  ng-trim="{{doTrim}}"\
  *        в function MdAutocomplete () в scope добавить  doTrim: '@mdTrim',
  *        В шаблоне у <md-autocomplete> добавить md-trim="false"
  * 2) При удалении чипа позиция списка предположений не сдвигается по горизонтали.
  *  Патч: в function MdAutocompleteCtrl ($scope,...  в  //-- public methods  добавить
  *           ctrl.positionDropdown = positionDropdown;
  *        Пользоваться этим в колбэке onChipRemove(), см. реализацию ниже
  * 3) В моб. версии нельзя убрать фокус, когда видны предположения
  *  Патч: в шаблоне function getInputElement () добавить  is-blurable="{{isBlurable}}"\ can-scroll-around="{{canScrollAround}}"\
  *        в function MdAutocomplete () в scope добавить  isBlurable: '@mdBlurable', canScrollAround: '@mdCanScrollAround',
  *        в function handleHiddenChange (.. перед  $mdUtil.disableScrollAround(elements.ul);  вставить  if(ctrl.scope.canScrollAround === undefined || ctrl.scope.canScrollAround == 'false')
  *        в function onListEnter () и function onListLeave ()  добавить в начало
  *          if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) return;
  *          (иначе после клика на предположение поле сможет расфокуситься только после двух кликов, а не одного)
  *        в self.handler('focus'..  класса MdGesture  после if (canFocus(ev.target)) { .. } добавить
  *  		} else {
  *  			if(document.activeElement && document.activeElement.getAttribute('is-blurable') == 'true') {
  *  				document.activeElement.blur();
  *  			}
  *  		}
  *        В шаблоне у <md-autocomplete> добавить md-blurable="true", md-can-scroll-sround="false"
  * 4) В моб. версии при появлении клавиатуры поле ввода и дропдаун не всегода полностью в видимой области
  *  Патч: см. adoptWinScroll()
  *
  * Все патчи помечены комментариями "@ukrbublik".
  * Их нужно сохранить в случае обновления angular-material
  */
  ...
  var $mdUtil = {
    ...
    disableScrollAround: function(element, parent) {
      ...
      function disableBodyScroll() {
        ...
        return function restoreScroll() {
          // @ukrbublik {
          scrollOffset = $mdUtil.getRealScrollOffset();
          // } @ukrbublik
    ...
    // @ukrbublik {
    isBodyScrollingDisabled: function() {
      var body = $document[0].body;
      return body.style.position === 'fixed';
    },
    getRealScrollOffset: function() {
      var body = $document[0].body;
      if(!this.isBodyScrollingDisabled()) {
        return $mdUtil.scrollTop(body);
      } else {
        return -1 * parseInt(body.style.top);
      }
    },
    setRealScrollOffset: function(scrollOffset) {
      var body = $document[0].body;
      if(!this.isBodyScrollingDisabled()) {
...
function MdAutocompleteCtrl ($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
  ...
  // @ukrbublik {
  ctrl.positionDropdown              = positionDropdown;
  ctrl.adoptWinScroll				         = adoptWinScroll;
  // } @ukrbublik
  ...
  function init () {
    ...
    $mdUtil.nextTick(function () {
	  ...
	  // @ukrbublik {
      if( $mdUtil.isMobile() ) {
        angular.element(elements.input).on('focus', function(ev) {
          if(ev instanceof FocusEvent) {
            var root = elements.root.getBoundingClientRect();
            adoptWinScroll();
            watchKeyboardAnimations(function onOpening(kbHeight, winHeight) {
              adoptWinScroll();
        ...
        angular.element(elements.input).on('blur', function(ev) {
          if(ev instanceof FocusEvent) {
          }
        });
      }
  ...
  // @ukrbublik {
  /**
   * Dirty keyboard animation watcher. Callbacks:
   ...
   */
  function watchKeyboardAnimations(onOpening, onOpened, onClosing, onClosed, isOpening, swh, wh, time, lastChTime) {
    var delay = 50;
    ...
    if(isOpening ? time < watchAniTime : lastChTime === undefined || time < watchAniTime) {
      setTimeout(function() {
        watchKeyboardAnimations(onOpening, onOpened, onClosing, onClosed, isOpening, swh, wh, time, lastChTime);
      }, delay);
    } else {
      if(isOpening)
  ...
  /**
   * For mobile devices input should be at top edge of client view when drop down and at bottom edge when drop up
   * It's bad idea to call this method during kb animation if body scroll is not locked (md-can-scroll-around="true")
   */
  function adoptWinScroll() {
    if(!$mdUtil.isMobile() || !elements)
      return;
    var vrect = elements.snap.getBoundingClientRect(),
      root = elements.root.getBoundingClientRect(),
      dropdown = elements.scrollContainer.getBoundingClientRect(),
      scrollOffset = null,
      dropDirection = null;
    if(!dropdown.height)
      return; //dropdown not visible
    var realScrollOffset = $mdUtil.getRealScrollOffset();
    if(dropdown.top == vrect.bottom) {
      dropDirection = 'down';
      scrollOffset = vrect.top + realScrollOffset - 5;
    } else if(dropdown.bottom == vrect.top) {
      dropDirection = 'up';
      scrollOffset = vrect.bottom + realScrollOffset - root.height + 5;
    }
    //drop up is preferable
    var canDropUp = (vrect.top + realScrollOffset >= (/*ITEM_HEIGHT*/ 48 * 4 + MENU_PADDING * 2));
    if(canDropUp && dropDirection == 'down' || dropDirection === null) {
      scrollOffset = vrect.bottom + realScrollOffset - root.height + 5;
    }
    if(scrollOffset < 0) scrollOffset = 0;
    if(scrollOffset !== null && scrollOffset != realScrollOffset) {
      $mdUtil.setRealScrollOffset(scrollOffset);
    }
    if(canDropUp && dropDirection == 'down' || dropDirection === null) {
      positionDropdown();
    }
  ...
  function positionDropdown () {
    ...
    // @ukrbublik { - using forceDropUp
    //drop up is preferable
    var realScrollOffset = $mdUtil.getRealScrollOffset();
    var canDropUp = (vrect.top + realScrollOffset >= (/*ITEM_HEIGHT*/ 48 * 4 + MENU_PADDING));
    var forceDropUp = canDropUp && $mdUtil.isMobile();
    // } @ukrbublik
    if (forceDropUp || top > bot && root.height - hrect.bottom - MENU_PADDING < MAX_HEIGHT) {
      styles.top       = 'auto';
  ...
  function handleHiddenChange (hidden, oldHidden) {
    if (!hidden && oldHidden) {
      ...
      if (elements) {
        $mdUtil.nextTick(function () {
		  // @ukrbublik {
		  if(ctrl.scope.canScrollAround === undefined || ctrl.scope.canScrollAround == 'false')
		  // } @ukrbublik
...
function MdAutocomplete () {
  ...
  return {
    ...
    scope:        {
      ...
      // @ukrbublik {
      doTrim:           '@mdTrim',
      isBlurable:       '@mdBlurable',
      canScrollAround:  '@mdCanScrollAround',
      // } @ukrbublik
    ...
    template:     function (element, attr) {
      ...
      added to template:
      ng-trim="{{doTrim}}"\
      is-blurable="{{isBlurable}}"\
      can-scroll-around="{{canScrollAround}}"\
      } @ukrbublik */
